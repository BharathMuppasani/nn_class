#!/bin/zsh
# Train CartPole DQN variants - each in its own Terminal window
# Usage: ./train_cartpole.sh [variants...]
# Examples:
#   ./train_cartpole.sh                    # Train all 5 variants
#   ./train_cartpole.sh dqn ddqn           # Train only DQN and Double DQN
#   ./train_cartpole.sh ddqn_duel_per      # Train only Double+Dueling+PER
...
launch_training() {
    ...
    local save_dir="../output/cartpole/${variant}"
...
tell application "Terminal"
    do script "cd ${CODE_DIR} && source ~/anaconda3/etc/profile.d/conda.sh && conda activate ${CONDA_ENV} && python train.py --env CartPole-v1 --episodes ${EPISODES} ${flags} --batch-size 128 --save-dir ${save_dir}; exit"
    activate
end tell
EOF
}
...
# Main
echo "=========================================="
echo "  CartPole DQN Training Launcher"
echo "=========================================="
...
if [[ $# -eq 0 ]]; then
    for v in "${ALL_VARIANTS[@]}"; do
        echo "  python plot_metrics.py --dirs ../output/cartpole/${v} --save-dir ../output/cartpole/${v}"
    done
    echo ""
    echo "  # Comparison plot"
    echo "  python plot_metrics.py --dirs ../output/cartpole/dqn ../output/cartpole/ddqn ../output/cartpole/ddqn_duel ../output/cartpole/ddqn_per ../output/cartpole/ddqn_duel_per --save-dir ../output/cartpole/comparison"
else
    for v in "$@"; do
        echo "  python plot_metrics.py --dirs ../output/cartpole/${v} --save-dir ../output/cartpole/${v}"
    done
    if [[ $# -gt 1 ]]; then
        ...
        dirs=""
        for v in "$@"; do
            dirs="${dirs} ../output/cartpole/${v}"
        done
        echo "  python plot_metrics.py --dirs${dirs} --save-dir ../output/cartpole/comparison"
    fi
fi
echo "=========================================="
